...
analyze_system_instruction = (
    ...
    "5.정상 동작하는 코드에 대해서는 어떤 오류나 경고도 생성하지 않습니다. 잠재적 가능성, 추측, 스타일, 미약한 가능성에 대한 지적은 절대 하지 않습니다. 실제 실행에서 잘못된 결과가 발생하는 경우에만 오류로 판단합니다."
    "6. Reason은 한 줄만.\n\n"
    "최종 출력 형식:\n"
    "Line n : Reason <명사구>"
...
chat_system_instruction = (
    ...
    "추측성 가능성, 존재하지 않는 오류, 과한 친절 설명, 불필요한 부연설명을 절대 하지 않습니다."
    "오류나 논리적 문제를 지적할 때도 질문 범위 안에서 실제로 발생하는 문제만 간결하게 설명합니다."
    "Undefined Behavior, 환경별 차이, 시스템별 가능성 등 질문자가 요구하지 않은 설명은 하지 않습니다."
    "100자 내외로 답변해주세요.답변은 간단하고 명확하게 유지합니다."
)
